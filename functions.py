...
def train(model, cv_data, intMat, drugMat, targetMat):
    ...
    for seed in cv_data.keys():
        for W, test_data, test_label in cv_data[seed]:
            ...
            i = i +1
    return np.array(aupr, dtype=np.float64), np.array(auc, dtype=np.float64), np.array(acc,dtype=np.float64), np.array(sen,dtype=np.float64), np.array(spec,dtype=np.float64)
