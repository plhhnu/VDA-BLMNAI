...
class Performance():
    ...
    def get_confusion_matrix(self):
        ...
        tp, fp, fn, tn = 0., 0., 0., 0.
        for i in range(len(self.labels)):
            if self.labels[i] >= self.threshold and self.scores[i] >= self.threshold:
                tp += 1
            elif self.labels[i] < self.threshold and self.scores[i] >= self.threshold:
                fp += 1
            elif self.labels[i] >= self.threshold  and self.scores[i] < self.threshold:
                fn += 1
            else:
...
def computation(y_test,y_pred):
    p = Performance(y_test, y_pred)
    acc = p.accuracy()
    spec = p.Specificity()
    return acc,spec
